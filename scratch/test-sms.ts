...
async function main() {
  ...
  try {
    ...
    if (errorMsg.includes('InvalidSenderId') || errorMsg.includes('402')) {
      console.log('   → Remove AT_SENDER_ID from your environment (set it to empty).');
    }
    if (errorMsg.includes('InsufficientBalance') || errorMsg.includes('405')) {
      console.log('   → Top up your Africa\'s Talking SMS balance.');
    }
    if (errorMsg.includes('InvalidPhoneNumber') || errorMsg.includes('403')) {
