  ...
  try {
    ...
    if (config) {
      ...
      if (config.isManuallyClosed) {
        return error(
          "Voter registration is currently suspended by the IEC.",
          403,
        );
      }
    ...
    if (voter.emailVerified && voter.email) {
      return error(
        "An email is already verified for this account. Contact the IEC to change it.",
        409,
      );
    }
    ...
    if (emailTaken) {
      return error(
        "This email is already linked to another voter account.",
        409,
      );
    }
    ...
    const { expiresAt, emailFailed } = await sendEmailRegistrationOTP(
      email,
      normalizedPhone,
      ipAddress,
    );
    ...
    return success({
      ...
      ...(emailFailed && {
        emailWarning:
          "OTP created but email delivery may have failed. Try requesting a new code.",
      }),
    });
  } catch (err) {
