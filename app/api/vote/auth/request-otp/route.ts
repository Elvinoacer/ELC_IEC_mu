...
import {
  findRecentReusableOTP,
  getOTPRateLimitState,
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!voter)
      return error("This number isn't in the ELP voter registry.", 404);
    if (voter.hasVoted) return error("You have already cast your vote.", 409);
    if (!voter.email || !voter.emailVerified) {
      return error(
        "This voter account does not have a verified email address for voting.",
        409,
      );
    }
    ...
    if (recentOtp) {
      const remainingSeconds = Math.max(
        1,
        Math.ceil((recentOtp.expiresAt.getTime() - Date.now()) / 1000),
      );
      return success({
        alreadySent: true,
    ...
    const { expiresAt, smsFailed } = await sendOTP(
      normalizedPhone,
      voter.email,
      ipAddress,
    );
