  ...
  try {
    ...
    for (const sel of selections) {
      if (seenPositionIds.has(sel.positionId)) {
        return error(
          `Multiple selections detected for the same position ID: ${sel.positionId}`,
          400,
        );
      }
      if (seenCandidates.has(sel.candidateId)) {
        return error(
          `Duplicate candidate selected across positions: ${sel.candidateId}`,
          400,
        );
      }
    ...
    if (missingIds.length > 0) {
      const missingTitles = activePositions
        .filter((p) => missingIds.includes(p.id))
        .map((p) => p.title);
      return error(
        `Incomplete ballot. You must vote for all positions: ${missingTitles.join(", ")}`,
        400,
      );
    }

    // Check if any selections are for positions that aren't active or don't exist
    const invalidIds = Array.from(seenPositionIds).filter(
      (id) => !activeIds.includes(id),
    );
    if (invalidIds.length > 0) {
      return error(
        `Invalid positions detected in ballot (IDs): ${invalidIds.join(", ")}`,
        400,
      );
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const voterData = await tx.$queryRaw<
        { id: number; has_voted: boolean; device_hash: string | null }[]
      >`
      ...
      const config = await tx.votingConfig.findFirst({
      ...
      const isDeviceMismatch =
        dbVoter.device_hash && deviceHash && dbVoter.device_hash !== deviceHash;
      if (isDeviceMismatch) {
        console.warn(
          `[VOTE_SIGNAL] Device mismatch for voter ${voterId}. Expected: ${dbVoter.device_hash}, Got: ${deviceHash}`,
        );
      }
      ...
      for (const sel of selections) {
        ...
        if (candidate.status !== "APPROVED")
          throw new Error(`CANDIDATE_NOT_APPROVED_${sel.candidateId}`);
        if (candidate.positionId !== sel.positionId)
          throw new Error(`POSITION_MISMATCH_${sel.candidateId}`);
    ...
    try {
      ...
      const broadcastUrl =
        ...
        "http://localhost:3001/internal/broadcast-results";
    ...
    let status: "SUCCESS" | "FAILED" | "DUPLICATE" | "OUTSIDE_WINDOW" =
      "FAILED";
    ...
    if (errorMsg === "VOTING_CLOSED_MANUALLY")
      return error("Voting has been manually closed by the IEC.", 403);
    if (errorMsg === "VOTING_NOT_STARTED")
      return error("Voting has not started yet.", 403);
    if (errorMsg === "VOTING_ALREADY_CLOSED")
      return error("Voting has already closed.", 403);

    if (
      errorMsg?.startsWith("INVALID_CANDIDATE") ||
      errorMsg?.startsWith("CANDIDATE_NOT_APPROVED") ||
      errorMsg?.startsWith("POSITION_MISMATCH")
    ) {
      return error(
        "Invalid candidate selection detected. Please refresh and try again.",
        400,
      );
    }
