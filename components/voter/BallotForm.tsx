...
export default function BallotForm({ positions, deviceHash }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Success: show confirmation page first.
    ...
    <div className="space-y-12">
        ...
        <div key={pos.id} className="fade-in">
          <div className="flex items-center gap-4 mb-4">
            ...
            {selections[pos.title] ? (
              <span className="text-xs font-medium px-2 py-1 bg-success-500/20 text-success-400 rounded-full border border-success-500/30">
                Selected
              </span>
            ) : (
              <span className="text-xs font-medium px-2 py-1 bg-error-500/20 text-error-400 rounded-full border border-error-500/30">
                Required
              </span>
            )}
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4 relative z-10">
                    <div
                      className={`w-16 h-16 rounded-full overflow-hidden shrink-0 transition-transform ${isSelected ? "scale-105 ring-2 ring-brand-500 ring-offset-2 ring-offset-surface-900" : ""}`}
                    >
                    ...
                    <div>
                      <h3
                        className={`font-bold ${isSelected ? "text-white" : "text-slate-200"}`}
                      >
                        {candidate.name}
                      </h3>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm fade-in">
          <Card
            padding="xl"
            className="w-full max-w-md bg-surface-800 border-brand-500/30"
          >
            <h2 className="text-2xl font-bold text-white mb-2 font-[family-name:var(--font-outfit)]">
              Confirm Your Vote
            </h2>
            <p className="text-sm text-slate-400 mb-6">
              You are about to cast your final ballot. This action{" "}
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                loading={loading}
                className="flex-1 shadow-[0_0_20px_rgba(37,99,235,0.4)]"
              >
                Cast Final Vote
              </Button>
            </div>
